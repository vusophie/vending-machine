...
export default function Step4Page({
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center space-y-6 p-6">
      ...
      <Card
        className="p-6 w-full max-w-md rounded-xl bg-gradient-to-r from-teal-400 to-blue-500"
        shadow="sm"
      >
      ...
      <div className="flex gap-4">
        <Button
          ...
          isDisabled={!coinOptions.quarter}
          onClick={() => handleCoinSelection("quarter")}
        >
          25¢
        </Button>
        <Button
          className="bg-blue-600 text-white px-6 py-2 rounded-lg shadow-md hover:bg-blue-700 transition"
          isDisabled={!coinOptions.dime}
          onClick={() => handleCoinSelection("dime")}
        >
          10¢
        </Button>
        <Button
          className="bg-blue-600 text-white px-6 py-2 rounded-lg shadow-md hover:bg-blue-700 transition"
          isDisabled={!coinOptions.nickel}
          onClick={() => handleCoinSelection("nickel")}
        >
          5¢
        </Button>
        <Button
          className="bg-blue-600 text-white px-6 py-2 rounded-lg shadow-md hover:bg-blue-700 transition"
          isDisabled={!coinOptions.none}
          onClick={() => handleCoinSelection("none")}
        >
          0¢
        </Button>
      </div>
        ...
        <div className="mt-4">
          ...
          {Object.entries(selectedCoin).map(([coin, count]) =>
            count > 0 ? (
              <p key={coin}>
                {count} ×{" "}
                {coin === "nickel" ? "5¢" : coin === "dime" ? "10¢" : "25¢"}
              </p>
            ) : null,
          )}
          <Button
            className="bg-purple-600 text-white px-6 py-2 rounded-lg shadow-md hover:bg-purple-700 transition mt-4"
